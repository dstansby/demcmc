...
class TestContFuncDiscrete:
    def test_binned_error(self, cont_func):
        tbins = TempBins([0, 1, 3, 4] * u.K)
        with pytest.raises(
            ValueError,
            match=r"The following bin edges in temp_bins are missing from the contribution function temperature coordinates: \[<Quantity 0. K>\]",
        ):
            cont_func.binned(tbins)

    def test_binned(self, cont_func):
        tbins = TempBins([1, 3, 5] * u.K)
